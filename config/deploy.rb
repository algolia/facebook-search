...
set :default_environment, {
  'GIT_SSH' => '/home/prod/ssh-facebook-search.sh'
}
...
# application
set :application, "Facebook Search"
set :deploy_to, "/var/www/facebook-search"
set :user, "prod"
set :use_sudo, false

# repository
set :local_repository, "."
set :repository, "git@github.com:algolia/facebook-search.git"
set :scm, :git
...
namespace :bluepill do
  ...
  task :quit, :roles => [:cron] do
    run "cd #{current_path} && bundle exec bluepill facebook-search --no-privileged stop"
    run "cd #{current_path} && bundle exec bluepill facebook-search --no-privileged quit"
  end

  desc "Load bluepill configuration and start it"
  task :start, :roles => [:cron] do
    run "cd #{current_path} && bundle exec bluepill --no-privileged load /var/www/facebook-search/current/config/production.pill"
  end

  desc "Prints bluepills monitored processes statuses"
  task :status, :roles => [:cron] do
    run "cd #{current_path} && bundle exec bluepill facebook-search --no-privileged status"
  end
end
